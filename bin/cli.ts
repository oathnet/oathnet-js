...
const bulk = program.command('bulk').description('Bulk search commands');

bulk
  .command('create')
  .description('Create a bulk search job')
  .requiredOption('-t, --term <terms...>', 'Search terms')
  .requiredOption('--service <service>', 'Service (breach|stealer)')
  .option('--format <format>', 'Output format', 'jsonl')
  .action(async (opts) => {
    try {
      const client = getClient();
      const result = await client.bulk.create(opts.term, opts.service, {
        format: opts.format,
      });
...
bulk
  ...
  .action(async (jobId) => {
    try {
      const client = getClient();
      const result = await client.bulk.getStatus(jobId);
      ...
      output(result, (r) => {
        if (r.data) {
          ...
          console.log(`Job ID: ${r.data.job_id}`);
          console.log(`Status: ${r.data.status}`);
          console.log(`Terms: ${r.data.terms_count}`);
          console.log(`Results: ${r.data.results_count}`);
        }
      });
    } catch (error) {
      handleError(error);
...
bulk
  .command('list')
  .description('List bulk search jobs')
  .action(async () => {
    try {
      const client = getClient();
      const result = await client.bulk.list();

      output(result, (r) => {
        console.log(chalk.bold('\nBulk Jobs\n'));
        if (r.results?.length) {
          r.results.forEach((j: any) => {
            console.log(`  ${j.job_id}: ${j.status} (${j.terms_count} terms)`);
          });
        } else {
...
bulk
  .command('download')
  .description('Download bulk search results')
  .argument('<job_id>', 'Job ID')
  .requiredOption('-o, --output <file>', 'Output file path')
  .action(async (jobId, opts) => {
    try {
      const client = getClient();
      console.log(`Downloading bulk results ${jobId}...`);
      const path = await client.bulk.download(jobId, opts.output);
      console.log(chalk.green(`Downloaded to ${path}`));
    } catch (error) {
      handleError(error);
    }
